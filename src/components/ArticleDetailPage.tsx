...
export const ArticleDetailPage: React.FC<ArticleDetailPageProps> = ({
  ...
  const article =
    GLAMIRK_JOURNAL_ARTICLES_EXTENDED.find((a) => a.id === articleId) ||
  ...
  // Related articles
  const relatedArticles = GLAMIRK_JOURNAL_ARTICLES_EXTENDED.filter(
    (a) => article.relatedArticleIds?.includes(a.id) || (a.category === article.category && a.id !== article.id)
  ).slice(0, 3);

  // Related looks
  const relatedLooks = GLAMIRK_LOOKS.filter((l) =>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-20 pt-16 border-t border-[#E8D5A8] space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {relatedArticles.map((relArt) => (
            <div
              key={relArt.id}
              onClick={() => onOpenArticle(relArt.id)}
              className="group cursor-pointer bg-[#FAF9F6] border border-[#E8D5A8] p-6 hover:shadow-lg transition-all flex flex-col justify-between"
            >
              <div className="space-y-3">
                <div className="aspect-[16/10] overflow-hidden bg-[#FAF9F6]">
                  <img
                    src={relArt.image}
                    alt={relArt.title}
                ...
                <div className="flex items-center justify-between text-[11px] text-[#6B6B6B]">
                  <span>{relArt.category}</span>
                  <span>{relArt.readTime}</span>
                </div>
                <h4 className="font-serif text-lg text-[#121212] group-hover:text-[#C9972B] transition-colors leading-snug">
                  {relArt.title}
                </h4>
