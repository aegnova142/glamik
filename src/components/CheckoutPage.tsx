...
export const CheckoutPage: React.FC<CheckoutPageProps> = ({
  ...
  const handlePlaceOrder = () => {
    ...
    // Simulate luxury order verification & payment processing
    setTimeout(() => {
      setIsSubmitting(false);

      const generatedOrderNumber = `GLM-${Math.floor(10000 + Math.random() * 90000)}`;
      const now = new Date();
      ...
      const newOrder: Order = {
        ...
        items: cartItems.map((item) => ({
          ...
          price: item.selectedSize === '30g' ? 549 : item.product.price,
        ...
        timeline: [
          {
            status: 'PLACED',
            timestamp: 'Just now',
            note: 'Order confirmed and registered at Glamirk Atelier.',
            completed: true,
          },
          {
            status: 'PACKED',
            timestamp: 'Estimated within 4 hours',
            note: 'Handcrafted signature packaging with wax seal.',
            completed: false,
          },
          {
            status: 'SHIPPED',
            timestamp: 'Pending Dispatch',
            note: 'Handover to priority air courier.',
            completed: false,
          },
          {
            status: 'OUT_FOR_DELIVERY',
            timestamp: 'Pending',
            note: 'Doorstep assignment.',
            completed: false,
          },
          {
            status: 'DELIVERED',
            timestamp: 'Pending',
            note: 'Handover confirmation.',
            completed: false,
          },
        ],
      };
