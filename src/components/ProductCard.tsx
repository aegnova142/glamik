...
export const ProductCard: React.FC<ProductCardProps> = ({
  ...
  return (
    <div
      ...
      className="group relative flex flex-col justify-between bg-white rounded-2xl sm:rounded-3xl border border-[#E8D5A8] p-3 sm:p-4 shadow-[0_4px_16px_rgba(240, 90, 126,0.04)] hover:shadow-[0_12px_30px_rgba(240, 90, 126,0.12)] hover:border-[#FCE8ED] transition-all duration-300"
      onMouseEnter={() => setIsHovered(true)}
      onMouseLeave={() => setIsHovered(false)}
    >
      {/* Product Image Frame */}
      <div className="relative aspect-square sm:aspect-[4/4.5] w-full overflow-hidden rounded-xl sm:rounded-2xl bg-[#FCE8ED]">
        {/* Main Product Image */}
        <img
          src={displayImage}
          alt={product.name}
      ...
      {/* Product Information */}
      <div className="pt-3 pb-1 flex flex-col space-y-1.5 flex-1 justify-between">
        <div>
          {/* Subcategory & Star Rating */}
          <div className="flex items-center justify-between text-[11px] text-[#6B6B6B] font-medium">
            <span>{product.subCategory}</span>
            {product.rating && (
              <span className="flex items-center gap-1 text-amber-500 font-semibold text-xs">
                ★ {product.rating} <span className="text-[10px] text-[#6B6B6B]">({product.reviewCount})</span>
              </span>
            )}
          </div>
