...
export const FindMyShadePage: React.FC<FindMyShadePageProps> = ({
  ...
  // The <video> element only mounts once isCameraActive flips true, so the
  // stream must be attached after that render (a ref set inside startCamera
  // would still be null — the element doesn't exist yet).
  useEffect(() => {
    ...
    <div className="bg-[#FAF9F6] min-h-screen text-[#121212] pb-24">
      ...
      {currentStep === 0 && (
        <div>
          ...
          {shadeJourney && shadeJourney.steps.length > 0 && (
            <BeautyJourneySection journey={shadeJourney} onStepAction={() => setCurrentStep(1)} />
          )}
        </div>
      )}
      ...
      {currentStep === 6 && (
        <div className="max-w-3xl mx-auto px-4 sm:px-6 pt-12 pb-20 space-y-8">
          ...
          {uploadError && (
            <div className="p-3 bg-[#FCE8ED] border border-[#F05A7E]/40 text-center max-w-md mx-auto">
              <p className="text-xs text-[#F05A7E] font-medium">{uploadError}</p>
            </div>
          )}
              ...
              <div className="pt-6 border-t border-[#E8D5A8] space-y-3">
                ...
                <div className="flex items-center justify-between pt-2">
                  ...
                  >
                    {isProfileSaved ? (
                      <>
                        <Check className="w-3.5 h-3.5 text-[#C9972B]" />
